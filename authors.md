# Authors and Citation

## Authors

- **Pedro Baltazar**. Author, maintainer.

## Citation

Source:
[`DESCRIPTION`](https://github.com/pedrobtz/azr/blob/0.2.0/DESCRIPTION)

Baltazar P (2025). *azr: Credential Chain for Seamless 'OAuth 2.0'
Authentication to 'Azure Services'*. R package version 0.2.0,
<https://pedrobtz.github.io/azr/>.

    @Manual{,
      title = {azr: Credential Chain for Seamless 'OAuth 2.0' Authentication to 'Azure Services'},
      author = {Pedro Baltazar},
      year = {2025},
      note = {R package version 0.2.0},
      url = {https://pedrobtz.github.io/azr/},
    }
