...
* Added S7 classes `az_dataset`, `az_catalog`, and `az_dataset_manifest`, plus `az_dataset_uri()`, `az_dataset_resolve()`, `az_dataset_from_uri()`, `az_catalog_read()`, and `az_catalog_write()` for declaring and resolving Azure Storage datasets across tiers.
...
* Added `azure_spark_storage_conf()` for generating Spark/Hadoop configuration keys (prefixed `spark.hadoop.` by default) for `refresh_token`, `client_secret`, `managed_identity`, and `shared_key` authentication, with sovereign cloud support and a secret-redacting `print()` method.
* `parse_storage_path()` now accepts `az://`/`azure://` schemes, captures an `endpoint_suffix` for sovereign clouds, handles DNS-zone endpoints, and redacts SAS credentials when printed.
* Breaking: `credential_chain()` is now eager, validating and normalizing entries at definition time, and `get_credential_provider()` builds each entry from an explicit context instead of scraping its caller's frame; added `credential_spec(class, ...)` for per-entry constructor overrides, and renamed `get_credential_provider(interactive = )` to `allow_interactive = `.
