...
* Added S7 classes `az_dataset`, `az_catalog`, and `az_dataset_manifest` for declaring collections of Azure Storage datasets and sharing resolved dataset metadata with external packages, along with `az_dataset_from_uri()`, `az_catalog_read()`, `az_catalog_write()`, `dataset_uri()`, and `dataset_manifest()`. A catalog supports `[[`, `names()`, and `length()`, and `dataset_uri()`/`dataset_manifest()` dispatch on both `az_dataset` and `az_catalog` (with an optional `name` to look up a single dataset). `dataset_manifest()` returns typed manifests containing the `name`, `uri`, and `format`, e.g. for use with `sparklyr::spark_read_source()`. The `format` field accepts `delta`, `parquet`, `csv`, `tsv`, `json`, `avro`, `orc`, or `text`; `az_dataset_from_uri()` infers it from the URI when possible and errors with guidance when the URI is a directory. The default `tier` for `dataset_uri()`, `dataset_manifest()`, and `az_dataset_from_uri()` is controlled by the new `dataset_tier` option (`options(azr.dataset_tier = ...)` or `AZR_DATASET_TIER`, default `"prod"`). Adds `S7` to Imports (#noissue).
