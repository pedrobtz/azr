...
* Added S7 classes `az_dataset` and `az_catalog` for declaring collections of Azure Storage datasets, along with `az_dataset_from_uri()`, `az_catalog_read()`, `az_catalog_write()`, and `dataset_uri()`. A catalog supports `[[`, `names()`, and `length()`, and `dataset_uri()` dispatches on both `az_dataset` and `az_catalog` (with an optional `name` to look up a single dataset). The `format` field accepts `delta`, `parquet`, `csv`, `tsv`, `json`, `avro`, `orc`, or `text`; `az_dataset_from_uri()` infers it from the URI when possible and errors with guidance when the URI is a directory. The default `tier` for `dataset_uri()` and `az_dataset_from_uri()` is controlled by the new `dataset_tier` option (`options(azr.dataset_tier = ...)` or `AZR_DATASET_TIER`, default `"prod"`). Adds `S7` to Imports.
